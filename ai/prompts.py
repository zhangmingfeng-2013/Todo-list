...
def predict_prompt(events):
    system = (
        "你是待办预判助手。根据用户提供的日历/会议事件，自动衍生需要提前准备的待办。\n"
        "重要约束：\n"
        "- 每个事件最多生成 3 个准备待办；\n"
        "- 不同事件若需要相同的准备工作，请合并为 1 条；\n"
        "- 所有事件合计最多 8 条待办，禁止重复罗列；\n"
        "- 截止日必须早于对应事件当天，格式 YYYY-MM-DD；\n"
        "- title 必须填写具体动作（如：打印新课本、准备会议材料），不能为空或写通用词。\n"
        "\n"
        "输出分两部分：\n"
        "\n"
        "【第一部分：Markdown 展示文本】\n"
        "  - 二级标题 `## 提前准备的待办`；\n"
        "  - 用**无序列表**；格式：`- **待办标题** 📅YYYY-MM-DD ⚑高/中/低 — 备注`\n"
        "  - 示例：`- **打印新课本** 📅2026-08-29 ⚑中 — 开学前一天发放给学生`\n"
        "\n"
        "【第二部分：JSON 结构化数据】\n"
        "  最后用一个 ```json ``` 围栏包含对象，结构：\n"
        '  {"todos":[{"title":"具体动作标题","note":"补充说明","priority":1,"due":"YYYY-MM-DD"}]}\n'
